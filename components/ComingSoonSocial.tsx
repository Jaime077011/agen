...
export function ComingSoonSocial() {
  ...
  useEffect(() => {
    ...
    // Stop mouseover from bubbling to CursorTracker's document listener
    // so it never activates the generic arrow cursor on social links
    const socialEl = document.querySelector('.cs-social');
    function stopProp(e: Event) { e.stopPropagation(); }
    socialEl?.addEventListener('mouseover', stopProp);
    ...
    function tick() {
      ...
      if (bubbleRef.current) {
        bubbleRef.current.style.left = nx + 'px';
        bubbleRef.current.style.top  = ny + 'px';
      }
    ...
    return () => {
      socialEl?.removeEventListener('mouseover', stopProp);
  ...
  function hideSiblingCursors() {
    ...
    document.getElementById('cursor-link')?.classList.remove('active');
  }

  function showSiblingCursors() {
  ...
  return (
    <>
      {/* Custom social cursor bubble */}
      <div
        ref={bubbleRef}
        className={`cs-social-cursor${hovered ? ' active' : ''}`}
        aria-hidden="true"
      >
      ...
      {/* Social links row */}
      <div className="cs-social anim-fade" style={{ '--delay': '900ms' } as React.CSSProperties}>
        {LINKS.map(({ key, label, href }) => (
          <a
            ...
            aria-label={label}
            onMouseEnter={() => { setHovered(key); hideSiblingCursors(); }}
            onMouseLeave={() => { setHovered(null); showSiblingCursors(); }}
          >
            {ICONS[key]}
          </a>
        ))}
